...
export default function Home() {
  const [formData, setFormData] = useState({ username: undefined, role: "USER" });
  ...
  function createUser(e: FormEvent){
    ...
    UserService.createUser({user_name:formData.username, role:formData.role}).then(
      (token) => {
        if (token !== undefined){
            ...
            <div className={createUserHidden ? "hidden": "" +  "mt-2"}>
              <form onSubmit={createUser} className="w-full max-w-lg flex flex-wrap">
                <div className="px-3">
                  ...
                  <input id="username" type="text" placeholder="username" onChange={handleChange} className="block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 focus:outline-none focus:bg-white focus:border-gray-500"></input>
                </div>
                <div className="px-3">
